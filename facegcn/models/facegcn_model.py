...
from ..losses import build_my_loss


@MODEL_REGISTRY.register()
class FaceGCNModel(BaseModel):
    """Base FaceGCN model for real-world blind face restoratin """
    def __init__(self, opt):
        super(FaceGCNModel, self).__init__(opt)
        self.build_model()
    ...
    def setup_losses(self):
        ...
        # adj loss
        if train_opt.get("adj_opt"):
            self.cri_adj = build_my_loss(train_opt['adj_opt']).to(self.device)
        else:
            self.cri_adj = None

        #
        if train_opt.get("adj_struct_opt"):
            self.cri_adj_stru = build_my_loss(train_opt['adj_struct_opt']).to(self.device)
        else:
            self.cri_adj_stru = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, self.adj = self.net_g(self.lq, mode="train")
        ...
        if self.cri_adj:
            l_g_adj = self.cri_adj(self.adj)
            l_g_total += l_g_adj
            loss_dict['l_g_adj'] = l_g_adj

        if self.cri_adj_stru:
            l_g_adj_stru = self.cri_adj_stru(self.adj)
            l_g_total += l_g_adj_stru
            loss_dict["l_g_adj_struct"] = l_g_adj_stru
    ...
    def test(self):
        with torch.no_grad():
            ...
            self.output, self.adj = self.net_g(self.lq)
            # end_time = time.time()
